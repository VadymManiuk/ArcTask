...
export function isProtocolIntegrationTask({ title, text }) {
  const normalizedTitle = typeof title === "string" ? title : "";
  const normalizedText = typeof text === "string" ? text : "";
  return (
    /\b(integration|ingestion)\b/i.test(normalizedTitle) ||
    /\b(cctp|cross-chain|indexer|sdk|webhook)\b/i.test(normalizedText) ||
    /\b(?:api|protocol|wallet|contract) integration\b/i.test(normalizedText)
  );
}
