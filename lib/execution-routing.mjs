...
export function createExecutionPlan(input, options = {}) {
  ...
  if (aiAssessment) {
    ...
    const recommendedTierIndex = aiAssessment.recommendedTier
      ...
      : scoreTier;
    const aiTierIndex = Math.max(scoreTier, recommendedTierIndex);
    requiredTierIndex =
  ...
  const maxOutputTokens = aiAssessment
    ? clamp(aiAssessment.estimatedOutputTokens, 500, selectedTier.maxOutputTokens)
    : selectedTier.maxOutputTokens;
  const maxRequests = aiAssessment
    ? clamp(aiAssessment.maxRequests, 1, selectedTier.maxRequests)
    : selectedTier.maxRequests;
  const maxAttempts = Math.min(selectedTier.maxAttempts, maxRequests);
  const escalationTier = tierDefinitions[Math.min(tierDefinitions.length - 1, selectedTierIndex + 1)];
  ...
  return {
    ...
    maxAttempts,
    validationPasses: 0,
    escalationModel: maxAttempts > 1 && escalationTier.model !== selectedTier.model
      ? escalationTier.model
      : null,
